...
import { SECTION_HEADER_HEIGHT, SeatAreaCharts, getSeatsSummary } from '@/utils/learning-center/seats';
...
const SeatOverview: React.FC<SeatOverviewProps> = ({ area, areaSeats }) => {
  ...
  return (
    <View
      className="box-border flex-row items-center justify-between bg-card px-4"
    ...
    >
      ...
      <View className="flex-row items-center">
        ...
        <Text className="mr-2 text-sm text-text-secondary">{summary.available}</Text>
        <View className="mr-1.5 h-3 w-3 rounded-full bg-red-200" />
        <Text className="mr-2 text-sm text-text-secondary">{summary.occupied}</Text>
      </View>
    </View>
  );
};
