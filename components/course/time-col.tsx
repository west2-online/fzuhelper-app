...
const TimeCol: React.FC<TimeColProps> = ({ height, minItemHeight = SCHEDULE_ITEM_MIN_HEIGHT }) => {
  ...
  const handleTitleTextLayout = (event: any) => {
    // 检测文本是否会被截断：如果文本宽度超过容器宽度（32px）
    const textWidth = event.nativeEvent.lines[0]?.width || 0;
    if (textWidth > 28 && titleFontSizeLevel < FONT_SIZE_LEVELS.title.length - 1) {
  ...
  const handleTimeTextLayout = (event: any) => {
    // 检测文本是否会被截断：如果文本宽度超过容器宽度（32px）
    const textWidth = event.nativeEvent.lines[0]?.width || 0;
    if (textWidth > 28 && timeFontSizeLevel < FONT_SIZE_LEVELS.time.length - 1) {
