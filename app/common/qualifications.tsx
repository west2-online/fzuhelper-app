        ...
        <ScrollView className="flex-1 rounded-tr-4xl bg-card px-8" contentContainerClassName="pt-8">
          <SafeAreaView edges={['bottom']}>
            ...
            <LabelEntry
              leftText="营业执照"
              rightText="点击查看"
              onPress={() => Linking.openURL(URL_BUSINESS_LICENSE)}
            />
            <LabelEntry
              leftText="学生组织证明"
              rightText="点击查看"
              onPress={() => Linking.openURL(URL_ORGANIZATION_PROOF)}
            />
            <LabelEntry
              leftText="域名注册信息"
              rightText="点击查看"
              onPress={() => Linking.openURL(URL_WEBSITE_PROOF)}
            />
            <LabelEntry
              leftText="安全检查报告"
              description="由福州大学网络安全与信息化办公室提供网络安全等级保护合规检测服务"
              rightText="点击查看"
              onPress={() => Linking.openURL(URL_SECURITY_CHECK_RESULT)}
            />
