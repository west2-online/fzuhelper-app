...
// Launcher declarations are generated at build time; the Expo Module handles Wants and visibility at runtime.
module.exports = config => {
  const bundleName = config.harmony.bundleName;
  config = registerHarmonyConfigPlugin(config, 'fzuhelper-quick-actions');
  config = withHarmonyDangerousMod(config, mod => {
    const projectDir = mod.modRequest.projectRoot;
    const resources = path.join(mod.modRequest.platformProjectRoot, 'entry/src/main/resources');
    const quickActions = syncHarmonyQuickActions({
      abilityName: 'EntryAbility',
      bundleName,
      moduleName: 'entry',
      configPath: path.join(projectDir, 'config/quick-actions.json'),
      projectDir,
      resourceRoot: path.join(resources, 'base'),
    });
    const catalogPath = path.join(resources, 'rawfile/expo-quick-actions.json');
    mkdirSync(path.dirname(catalogPath), { recursive: true });
    writeFileSync(catalogPath, JSON.stringify(quickActions, null, 2) + '\n');
    for (const relative of [
      'rawfile/expo-quick-actions.json',
      'base/element/expo_quick_actions.json',
      'base/profile/expo_quick_actions.json',
      ...quickActions.map((_, index) => `base/media/expo_quick_action_${index}.png`),
    ]) {
      recordManagedFile(mod, path.join(resources, relative), 'fzuhelper-quick-actions');
    }
    return mod;
  });
  return withModuleJson(config, mod => {
    const main = mod.modResults.module.abilities.find(ability => ability.name === 'EntryAbility');
    main.metadata = [
      ...(main.metadata ?? []).filter(item => item.name !== 'ohos.ability.shortcuts'),
      { name: 'ohos.ability.shortcuts', resource: '$profile:expo_quick_actions' },
    ];
    return mod;
  });
};
