...
/** 校历 这个不需要登录可以考虑做一点缓存 GET /api/v1/common/user/school-calendar */
...
/** [测试用] 登录 因为后续传参都是设置为identifier 和 cookies，但正常途径不方便获取这两个参数。这个接口的作用就是帮忙自动登录一次教务处，以获取id 和 cookies本地发送一次这个接口请求后，后续接口都会自动填充 id 和 cookies，不需要手动填写 GET /api/v1/internal/user/login */
...
/** 获取个人信息 后面如果有别的需求的话在返回字段接着添加，目前这些够了 GET /api/v1/jwch/user/info */
...
/** 验证码识别 POST /api/v1/user/validate-code */
